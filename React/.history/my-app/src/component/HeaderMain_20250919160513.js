...
const HeaderMain = () => {
  const { user, setUser, photos } = useAuth();
  return (
    <div className="flex items-center justify-center p-4 bg-white shadow-md">
      ...
      <div className="flex items-center gap-5 ml-auto mr-5">
        <span className="relative w-8">
          ...
          <span
            className="absolute top-0 right-0 flex items-center justify-center w-5 h-5 text-xs text-white bg-pink-600 rounded-full translate-x-2/4 "
          >
            {/* {cartItemsCount || 0} */}
          </span>
        </span>
      ...
      <button
        className="p-2 ml-auto text-black bg-gray-400 rounded-lg cursor-pointer"
