...
const MovieSearchApp = () => {
  ...
  const debounceValue = useDeboune(query, 500);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      const response = await axios.get(
        `https://api.themoviedb.org/3/movie/popular?api_key=03353b943970dc693fa2f337d37459be`
      );
      const response2 = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=03353b943970dc693fa2f337d37459be&query='${debounceValue}'`
      );
      if (debounceValue.trim() !== "") {
        setMovies(response2.data.results);
        setLoading(false);
      } else {
