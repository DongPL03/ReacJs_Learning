...
const PhotoList = () => {
  const { photos, toggleFavorite } = useGallery();
  return (
...
const PhotoItem = ({ info: { url, isFavorite } }) => {
  return (
    <div className="relative h-[300px] cursor-pointer group">
      <img src={url} alt="" className="object-cover w-full h-full" />
      <span className="absolute z-10 w-8 transition-all opacity-0 cursor-pointer right-5 top-5 group-hover:opacity-100 group-hover:visible">
        <svg
          width="42"
          height="38"
          viewBox="0 0 42 38"
      ...
      <button className="absolute invisible p-4 px-6 py-2 text-sm font-medium text-black transition-all bg-white rounded-lg opacity-0 cursor-pointer bottom-5 left-2/4 -translate-x-2/4 group-hover:opacity-100 group-hover:visible">
        Add to cart
      </button>
    </div>
  );
