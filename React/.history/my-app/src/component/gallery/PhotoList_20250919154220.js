...
const PhotoItem = ({ info: { url, isFavorite, id } }) => {
  ...
  return (
    <div className="relative h-[300px] cursor-pointer group">
      ...
      <button className="absolute invisible p-4 px-6 py-2 text-sm font-medium text-black transition-all bg-white rounded-lg opacity-0 cursor-pointer bottom-5 left-2/4 -translate-x-2/4 group-hover:opacity-100 group-hover:visible" onClick={}>
        Add to cart
      </button>
    </div>
  );
