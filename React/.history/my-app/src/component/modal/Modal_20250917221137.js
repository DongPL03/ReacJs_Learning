...
const Modal = ({ open = false, handleClose = () => {} }) => {
  ...
  return ReactDom.createPortal(
    <div className={`fixed inset-0 z-50 flex items-center justify-center invisible opacity-0 modal ${open ? "" : "opacity-0 invisible" }`}>
      ...
      <div className="modal-content bg-white z-10 relative p-10 rounded-lg w-full max-w-[482px]">
        <span
          className="absolute top-0 right-0 flex items-center justify-center w-10 h-10 p-1 rounded-full cursor-pointer -translate-y-2/4 translate-x-2/4"
          onClick={handleClose}
        >
          <svg
            width="14"
        ...
        <div className="flex flex-col gap-3 mb-5">
            ...
            <input type="email" placeholder="Enter your email" className="w-full text-sm leading-normal bg-[#e7ecf3] rounded-lg p-4"/>
        </div>
         <div className="flex flex-col gap-3 mb-5">
            <label htmlFor="password" className="text-sm cursor-pointer">Password</label>
            <input type="password" placeholder="Enter your password" className="w-full text-sm leading-normal bg-[#e7ecf3] rounded-lg p-4"/>
        </div>
